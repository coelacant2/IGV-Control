...
class Config():
    ...
    def __init__(self):
        ...
        self.speed_cost_gain = 0.75
        self.obstacle_cost_gain = 0.75
...
def getSpeedAndYaw(fSYaw, motion, goal, objects):
    global previousForwardYaw
    config = Config()

    ob = np.array(objects)[:,[0,1]]

    previousForwardYaw, ptraj = dwa_control(motion, previousForwardYaw, config, goal, ob)

    plt.cla()
    plt.plot(ptraj[:, 0], ptraj[:, 1], "-g")
    plt.plot(motion[0], motion[1], "xr")
    plt.plot(goal[0], goal[1], "xb")
    plt.plot(ob[:, 0], ob[:, 1], "ok")
    plot_arrow(motion[0], motion[1], motion[2])
    plt.axis("equal")
    plt.grid(True)
    plt.pause(0.01)

    return previousForwardYaw
